    ...
    public void start( Future<Void> startFuture ) throws Exception
    {
        ...
        vertxFileSystem.readFile( "openapi.json", readFile -> {
            if ( readFile.succeeded() )
            {
                ...
                Router swaggerRouter = SwaggerRouter
                        .swaggerRouter( router, swagger, vertx.eventBus(), new OperationIdServiceIdResolver() );

                deployVerticles( startFuture );

                vertx.createHttpServer().requestHandler( swaggerRouter::accept ).listen( getServerPort(), h -> {
                    if ( h.succeeded() )
                    {
